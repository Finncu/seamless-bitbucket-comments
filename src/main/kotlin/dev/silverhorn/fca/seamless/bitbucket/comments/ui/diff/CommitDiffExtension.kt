...
class CommitDiffExtension : DiffExtension() {
    ...
    override fun onViewerCreated(
        ...
        // Derive commit hashes from the request title / content description.
        // The title is the most reliable string available without internal API access.
        val title = request.title ?: return
        // Parse hashes from VCS diff requests: title contains "space hash (?)" in Git4Idea.
        val hashes = extractHashesFromTitle(title)
        val toHash = hashes.second ?: return
        val fromHash = hashes.first ?: ""

        // File path: take the last segment of the title.
        val filePath = title.substringAfterLast("/").substringBefore(" ").trim()
    ...
    private fun installUi(
        ...
        // Filter inline comments for this specific file.
        val fileComments = comments.filter { it.anchor?.path == filePath }
    ...
    private fun extractHashesFromTitle(title: String): Pair<String?, String?> {
        val sha1Pattern = Regex("[0-9a-f]{40}")
        val matches = sha1Pattern.findAll(title).map { it.value }.toList()
        return when {
